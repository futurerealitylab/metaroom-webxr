"use strict"
 ...
 let drawPlayer = (state, avatar) => {
   ...
   if (avatar.mode == MR.UserType.vr) {
      ...
      drawHeadset(state, hpos, headsetRot);
      drawController(state, rpos, rcontroller.orientation, 0);
      drawController(state, lpos, lcontroller.orientation, 1);
   }
   else {
      m.save();
 ...
 let drawHeadset = (state, position, orientation) => {
    m.save();
       ...
       m.save();
          m.scale(1,1.5,1);
          drawShape(state, CG.sphere, [0,0,0]);
       m.restore();
       for (let s = -1 ; s <= 1 ; s += 2) {
          m.save();
             m.translate(s*.4,.2,-.8);
             m.scale(.4,.4,.1);
             drawShape(state, CG.sphere, [10,10,10]);
          m.restore();
       }
    m.restore();
 }
 ...
 let drawController = (state, pos, rot, hand, isPressed) => {
    m.save();
       m.translate(pos);
       m.rotateQ(rot);
       ...
       m.save();
             ...
             drawShape(state, CG.sphere, isPressed ? [10,0,0] : [.5,0,0]);
       m.restore();
       m.save();
             m.translate(0,0,-.01).scale(.04,.04,.13);
             drawShape(state, CG.torus1, [0,0,0]);
       m.restore();
       m.save();
             m.translate(0,-.0135,-.008).scale(.04,.0235,.0015);
             drawShape(state, CG.cylinder, [0,0,0]);
       m.restore();
       m.save();
