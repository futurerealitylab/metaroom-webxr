...
//      console.log("there are " + n + " items in the scene");
   ...
   this.drawWithHierachy = (drawFunction, m) => {
      for (let i = 0 ; i < n ; i++) {
         let item = items[i];
         let mat = CG.matrixMultiply(m, item.matrix);
         mat = CG.matrixMultiply(mat, CG.matrixTranslate(item.mx, item.my, item.mz));
         mat = CG.matrixMultiply(mat, CG.matrixRotateX  (item.rx));
         mat = CG.matrixMultiply(mat, CG.matrixRotateY  (item.ry));
         mat = CG.matrixMultiply(mat, CG.matrixRotateZ  (item.rz));
...
let mTorus           = () => renderList.add(CG.torus);
let mDisk            = () => renderList.add(CG.disk);
let mCone            = () => renderList.add(CG.cone);
let mTube            = () => renderList.add(CG.tube);
let mGluedCylinder   = () => renderList.add(CG.gluedCylinder);
