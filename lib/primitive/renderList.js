...
let matrixBuffer = CG.matrixIdentity();
...
export let RenderList = function() {
   let Item = function() {
      ...
      this.init = () => {
         ...
         CG.setMatrixIdentity(this.matrix);
         CG.setMatrixIdentity(this.auxMatrix);
   ...
   this.addList = (list, drawFunction) => {
      if (items[n])
         items[n].init();
      else
         items[n] = new Item();
      ...
      items[n].auxMatrix = CG.matrixIdentity();
      items[n].type = 1;
      items[n].drawFunction = drawFunction;
      return items[n++];      
   }
   ...
   this.endFrame = drawFunction => {
      for (let i = 0 ; i < n ; i++) {
         ...
         CG.matrixMultiplyWithBuffer(buf0, mat, 
            CG.matrixTranslateComponentsWithBuffer(
               CG.translationBuffer, item.mx, item.my, item.mz)
         );
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateXWithBuffer(CG.rotateXBuffer, item.rx));
         CG.matrixMultiplyWithBuffer(buf0, buf1, CG.matrixRotateYWithBuffer(CG.rotateYBuffer, item.ry));
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateZWithBuffer(CG.rotateZBuffer, item.rz));
         mat = CG.matrixMultiplyWithBuffer(item.auxMatrix, buf1, 
               CG.matrixScaleNonUniformWithBuffer(
                  CG.scaleBuffer, item.sx, item.sy, item.sz)
         );
         ...
         case 1: {
            ...
            list.drawWithGlobalMatrix(mat, item.drawFunction);
            break;
         }
         default: {
            drawFunction(item, item.shape, mat, item.rgb, item.opac, item.textureInfo, item.fxMode, item.vertexMode);
            break;
         }
         }
      }
//      console.log("there are " + n + " items in the scene");
   ...
   this.drawWithGlobalMatrix = (globalMat, drawFunction) => {
      for (let i = 0 ; i < n ; i++) {
         ...
         CG.matrixMultiplyWithBuffer(buf0, buf1, 
            CG.matrixTranslateComponentsWithBuffer(
               CG.translationBuffer, item.mx, item.my, item.mz)
         );
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateXWithBuffer(CG.rotateXBuffer, item.rx));
         CG.matrixMultiplyWithBuffer(buf0, buf1, CG.matrixRotateYWithBuffer(CG.rotateYBuffer, item.ry));
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateZWithBuffer(CG.rotateZBuffer, item.rz));
         let mat = CG.matrixMultiplyWithBuffer(item.auxMatrix, buf1, 
               CG.matrixScaleNonUniformWithBuffer(
                  CG.scaleBuffer, item.sx, item.sy, item.sz)
         );
...
//      console.log("there are " + n + " items in the scene");
   ...
   this.getItems = () => { return { list : items, count : n }; };
}
...
export let mFoo             = () => activeList.add(CG.foo);
...
export function mList(list, drawFunction) {
   activeList.addList(list, drawFunction);
};
