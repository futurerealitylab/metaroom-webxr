...
export let RenderList = function() {
   ...
   this.endFrame = drawFunction => {
      for (let i = 0 ; i < n ; i++) {
         ...
         CG.matrixMultiplyWithBuffer(buf0, mat, CG.matrixTranslate(item.mx, item.my, item.mz));
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateX  (item.rx));
         CG.matrixMultiplyWithBuffer(buf0, buf1, CG.matrixRotateY  (item.ry));
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateZ  (item.rz));
         mat = CG.matrixMultiplyWithBuffer(item.auxMatrix, buf1, CG.matrixScale    (item.sx, item.sy, item.sz));
...
//      console.log("there are " + n + " items in the scene");
   ...
   this.drawWithGlobalMatrix = (globalMat, drawFunction) => {
      for (let i = 0 ; i < n ; i++) {
         ...
         CG.matrixMultiplyWithBuffer(buf0, buf1, CG.matrixTranslate(item.mx, item.my, item.mz));
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateX  (item.rx));
         CG.matrixMultiplyWithBuffer(buf0, buf1, CG.matrixRotateY  (item.ry));
         CG.matrixMultiplyWithBuffer(buf1, buf0, CG.matrixRotateZ  (item.rz));
         let mat = CG.matrixMultiplyWithBuffer(item.auxMatrix, buf1, CG.matrixScale    (item.sx, item.sy, item.sz));
