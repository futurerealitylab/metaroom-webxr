...
window.Shader = (function() {
    ...
    _util.addShaderModule = addShaderModule;

    function compileValidateStrings(vertSrc, fragSrc, errRecord) {
        const program = gl.createProgram();

        const vshader = Shader.addShaderModule(program, gl.VERTEX_SHADER, vertSrc, errRecord);
        const fshader = Shader.addShaderModule(program, gl.FRAGMENT_SHADER, fragSrc, errRecord);
    ...
    _util.compileValidateStrings = compileValidateStrings;

    function shaderLinkModules(vshader, fshader) {
        const program = gl.createProgram();
    ...
    _util.shaderLinkModules = shaderLinkModules;

    function shaderCompileValidateStringsEX(vertSrc, fragSrc) {
        const program = gl.createProgram();
        const vshader = Shader.addShaderModule(program, gl.VERTEX_SHADER, vertSrc);
        const fshader = Shader.addShaderModule(program, gl.FRAGMENT_SHADER, fragSrc);
    ...
    _util.shaderCompileValidateStringsEX = shaderCompileValidateStringsEX;
    ...
    function getAttributeLocations(_gl, program, data, suffix = "Loc") {
    ...
    _util.getAttributeLocations = getAttributeLocations;

    function getUniformLocations(_gl, program, data, suffix = "Loc") {
    ...
    _util.getUniformLocations = getUniformLocations;
    ...
    _util.scc = _util.preprocessShaderSource;
    ...
    function glAttachResourceTracking(GL, version) {
        ...
        for (let i = 0; i < len; i += 1) {
            ...
            GL[funcName] = function(arg) {
                ...
                Shader.resourceDeletionQueue.push(function() {
    ...
    function glFreeResources(GL) {
        ...
        const Q = Shader.resourceDeletionQueue;
        const len = Q.length;
        for (let i = 0; i < len; i += 1) {
    ...
    function setErrRecordGetSetClear(callbackGet, callbackSet, callbackClear) {
         _util.getErrRecord   = callbackGet   || function() { return Shader.errRecord; };
         _util.setErrRecord   = callbackSet   || function(val) { Shader.errRecord = val; };
         _util.clearErrRecord = callbackClear || function() { Shader.errRecord = {}; };
    }
