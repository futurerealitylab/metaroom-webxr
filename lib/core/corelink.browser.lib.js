  ...
  control.connect = async (IP, port) => (new Promise(async (resolve, reject) => {
    ...
    control.request = async (data) => {
      ...
      return (new Promise(async (resolve, reject) => {
        ...
        control.ID += 1
        let retries = 5
        control.client.addEventListener('message', async (content) => {
  ...
  */
  ...
  async function connect(credentials, config) {
    return (new Promise(async (resolve, reject) => {
      ...
      if (debug) console.log('Terget IP:', connConfig.ControlIP)
      let conn = false
      connected = true
      // connect the client
  ...
  */
  ...
  async function subscribe(options) {
    return (new Promise(async (resolve, reject) => {
      ...
      const request = `{"function":"subscribe","receiverID":"${receiverStream.streamID}","streamID":${JSON.stringify(workOptions.streamIDs)},"token":"${token}"}`
  ...
  */
  ...
  async function unsubscribe(options) {
    return (new Promise(async (resolve, reject) => {
      ...
      const request = `{"function":"unsubscribe","receiverID":${receiverStream.streamID},"streamID":${JSON.stringify(workOptions.streamIDs)},"token":"${token}"}`
