...
export class VoIP_webrtc {
    ...
    initAudio(that) {
        ...
        that.audioContext.listener.setPosition(0.1, 0, 0);
        // that.audioContext.listener.orientationY(0);
        // that.audioContext.listener.orientationZ(-1);
        that.panner = new PannerNode(that.audioContext, {
            // equalpower or HRTF
            panningModel: 'HRTF',
    ...
    playAudio(that, stream, peerUuid) {
        ...
        var realAudioInput = new MediaStreamAudioSourceNode(that.audioContext, {
            mediaStream: stream
        });
        that.peerConnections[peerUuid].audioInputStream = realAudioInput;

        realAudioInput.connect(that.panner);
        that.panner.connect(that.audioContext.destination);
