...
function compileValidateStringsAndHandleErrors(vertex, fragment) {
    GFX.tempCompiledShader = GFX.compileValidateStrings(
        vertex,
        fragment
    );
    GFX.tempCompiledShaderDirty = true;
}
_out.compileValidateStringsAndHandleErrors = compileValidateStringsAndHandleErrors;

function preprocessAndCreateShaderProgramFromStringsAndHandleErrors(vertex, fragment, libMap, autoIncludes) {
    
    const vertRecord = GFX.preprocessShaderSource(vertex,   libMap);
    const fragRecord = GFX.preprocessShaderSource(fragment, libMap);
...
_out.loadShader = loadShader;
_out.loadAndRegisterShaderForLiveEditing = loadShader;
...
function onNeedsCompilationDefault(args, libMap, userData) {
    ...
    const errRecord = {};
    const program = GFX.compileValidateStrings(
        vertRecord.shaderSource, 
        fragRecord.shaderSource, 
        errRecord
    );
...
function registerShaderForLiveEditing(_gl, key, args, callbacks, options) {
    ...
    if ((options && (options.doCompilationAfterFirstSetup !== false)) || !options) {
        ...
        if (record.hasError) {
            ...
            const shaderProgram = GFX.compileValidateStrings(defaultErrorVertex, defaultErrorFragment);
            if (record.onAfterCompilation) {
...
export {_out as ShaderTextEditor};
