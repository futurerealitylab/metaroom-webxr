...
ShaderTextEditor.hookIntoGFXLib = hookIntoGFXLib;
    ...
    { //// watch files
        ...
        for (let i = 0; i < args.length; i += 1) {
            ...
            if (arg.path) {
                ...
                saveTo = path.getLocalPath(arg.path);
...
function saveShaderToFile(key, status = {}) {
    ...
    function enqueueWrite(q, text, path, opts) {
        //console.log("writing", text, "to", path.getLocalPath(relativePath));
            ...
            if (textE) {
                ...
                if (options && options.paths && options.paths[prop]) {
                    ...
                    saveTo = (options.useAbsolutePaths) ?
                                options.paths[prop] :
                                path.getLocalPath(options.paths[prop]);
    ...
    { //// watch files
        ...
        for (let prop in args) {
            ...
            if (options && options.paths && options.paths[prop]) {
                ...
                saveTo = path.getLocalPath(options.paths[prop]);
