...
export function initGLContext(target, contextNames, contextOptions) {
...
export function addShaderModule(program, type, src, errRecord) {
    ...
    if (! gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        ...
        errRecord = (errRecord) ? errRecord : errInfo.errRecord;
...
export function compileValidateStrings(vertSrc, fragSrc, errRecord) {
    const program = gl.createProgram();

    const vshader = addShaderModule(program, gl.VERTEX_SHADER, vertSrc, errRecord);
    const fshader = addShaderModule(program, gl.FRAGMENT_SHADER, fragSrc, errRecord);
...
export function shaderLinkModules(vshader, fshader) {
    const program = gl.createProgram();
...
export function shaderCompileValidateStringsEX(vertSrc, fragSrc) {
    const program = gl.createProgram();
    const vshader = addShaderModule(program, gl.VERTEX_SHADER, vertSrc);
    const fshader = addShaderModule(program, gl.FRAGMENT_SHADER, fragSrc);
...
export function getAttributeLocations(_gl, program, data, suffix = "Loc") {
...
export function getUniformLocations(_gl, program, data, suffix = "Loc") {
...
export function preprocessShaderSource(string, libMap, alreadyIncludedSet, directiveState) {
    if (!libMap) {
        libMap = new Map();
    }
...
export function glAttachResourceTracking(GL, version) {
    ...
    for (let i = 0; i < len; i += 1) {
        ...
        GL[funcName] = function(arg) {
            ...
            GL.resourceDeletionQueue.push(function() {
...
export function glFreeResources(GL) {
    ...
    const Q = GL.resourceDeletionQueue;
    const len = Q.length;
    for (let i = 0; i < len; i += 1) {
...
export function setErrRecordGetSetClear(callbackGet, callbackSet, callbackClear) {
     errInfo.getErrRecord   = callbackGet   || function() { return errInfo.errRecord; };
     errInfo.setErrRecord   = callbackSet   || function(val) { errInfo.errRecord = val; };
     errInfo.clearErrRecord = callbackClear || function() { errInfo.errRecord = {}; };
}
...
export function setupVideo(url, args) {
    let copyVideo = false;
...
export function initVideoTexture(gl, textureSlot) {
