...
import {VoIP_holodeck} from "./voip_holodeck.js";
import {VoIP_webrtc} from "./voip_webrtc.js";
...

{
    ...
    window.PORT        = (PORT_ELEMENT && PORT_ELEMENT.getAttribute("value")) || "3000";
...
MR.initWebSocket = () => {
    ...
    try {
        MR.server.sock = new WebSocket(  
            "wss://" + window.IP + ":" + 3000//window.PORT
            );
    } catch (err) {
        console.log(err);
    }
...
// 
// MR.voip = new VoIP();
window.initVoip = function(username){
    // MR.voip = new VoIP_holodeck(username);
    MR.voip = new VoIP_webrtc(MR.server.sock, username);
}
