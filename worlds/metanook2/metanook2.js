...
function onStartFrame(t, state) {
    ...
    if (state.LC) {
       ...
       let D  = [LP[0] - RP[0], LP[1] - RP[1], LP[2] - RP[2]];
       let d = metersToInches(Math.sqrt(D[0] * D[0] + D[1] * D[1] + D[2] * D[2]));
       let getX = C => {
          m.save();
             m.identity();
...
function onDraw(t, projMat, viewMat, state, eyeIdx) {
    ...
    if (state.LC) {
       drawController(state.LC, [1,0,0]);
       drawController(state.RC, [0,1,1]);
