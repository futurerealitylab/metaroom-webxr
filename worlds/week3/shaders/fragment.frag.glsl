...
uniform vec3  uCursorClipspace; // cursor in -1 to 1 coordinates (z == 1.0 means down, 0.0 up)
uniform vec3  uCursorDir;       // cursor direction
...
void main() {
    ...
    for (int i = 0; i < 5; i += 1) {
       vec2 diff = ((uCursorClipspace.xy - (float(i) * uCursorDir.xy * 0.25)) - vPos.xy);
    ...
    float dtprod = dot(uCursorDir.xy, vPos.xy - uCursorClipspace.xy);
