...
async function loadImages(w) {
  ...
  try {
    ...
    w.textures = Tex.makeIndividualTexture2DsWithImages(
      ...
      images, ["wood", "brick", "tiles", "stones", "stones_bump", "rug1", "concrete", "woodFloor", "background", "wood1", "marble", "concrete1", "concrete2", "concrete3", "concrete4", "matisse", ],
      0
    );
...
function drawScene(time, w) {
  ...
  //QUICK NAVIGATION USING KEYS IN WE MODE: ← LEFT, → RIGHHT, ↑ FORWARD, ↓ BACK
  ...
  //QUICK NAVIGATION USING KEYS IN WE MODE: A TURN LEFT, D TURN RIGHT, W UP, S DOWN
  ...
  mCube().move(rw / 2 - rw / 32, rh / 2, rw / 2).size(rw / 16, rh / 2, .5).color(white).textureView(w.textures[11].lookupImageByID(1)).textureAtlas(w.textures[11]); // WALL
  mCube().move(-rw / 2 + rw / 32, rh / 2, rw / 2).size(rw / 16, rh / 2, .5).color(white).textureView(w.textures[14].lookupImageByID(1)).textureAtlas(w.textures[14]); // WALL
  mCube().move(0, rh - rh / 16, rw / 2 + 0.01).size(rw / 2, rh / 16, .5).color(white).textureView(w.textures[12].lookupImageByID(1)).textureAtlas(w.textures[12]); // WALL
  mCube().move(0, rh / 16, rw / 2 + 0.01).size(rw / 2, rh / 16, .5).color(white).textureView(w.textures[13].lookupImageByID(1)).textureAtlas(w.textures[13]); // WALL

  mCube().move(0, rh / 2, -rw / 2 + 0.001).turnZ(-Math.PI / 2).size(1.6, 2.4, .0015).color(superWhite).textureView(w.textures[15].lookupImageByID(1)).textureAtlas(w.textures[15]); // PAINTING
  mCube().move(0, rh / 2, -rw / 2 + 0.001).turnZ(-Math.PI / 2).size(1.68, 2.48, .0005).color(0.25, 0.2, 0.2).textureView(w.textures[9].lookupImageByID(1)).textureAtlas(w.textures[9]); // PAINTING FRAME
  ...
  mCube().move(-rw / 2, rh / 2, 0).size(.001, rh / 2, rw / 2).color(white).textureView(w.textures[6].lookupImageByID(1)).textureAtlas(w.textures[6]); // WALL
  mCube().move(0, rh, 0).size(rw / 2, .001, rw / 2).color(white).textureView(w.textures[6].lookupImageByID(1)).textureAtlas(w.textures[6]); // CEILING
  ...
  mRoundedCylinder().turnX(Math.PI / 2).move(-(sw / 4 + .5), sth, 0).size(stw, stw, sth).color(0.3, 0.3, 0.3).textureView(w.textures[10].lookupImageByID(1)).textureAtlas(w.textures[10]).fx(3); // SEAT
  ...
  mVase().turnX(-Math.PI / 2).move(0, th + 0.8, 0).color(0.01,0.01,0.01).size(0.4).fx(1);

  // SCULPTURES : CURRENTLY JUST SOME CONES
  mCone().turnX(Math.PI/2).move(8.,1.,-8).size(0.25,0.25,1.4);
  mCone().turnX(Math.PI/2).move(7.2,0.8,-8).size(0.3,0.3,0.8).color(0.2,0.1,0.1);
  mCone().turnX(Math.PI/2).move(6,1.,-8).size(0.3,0.3,1.).color(0.4,0.4,0.4).fx(1);

  // TEXT MATERIALS: fx(1) - PLASTER, fx(2) - METALLIC, fx(3) - GLOSSY RUBBER
  ...
  // CLOCK : CURRENTLY PROBLEMATIC, WILL FIX THIS PART AFTER ADDING HIERACHY TO ITEMS
  ...
  mRing().move(0, 3, 0).turnX(time).turnY(time).size(0.1, 0.1, .025).color(0.1, 0.1, 0.08).fx(2);
